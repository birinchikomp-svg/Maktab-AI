...
const AdminPanel: React.FC = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto animate-fadeIn text-slate-900">
        ...
        <div className="space-y-10 animate-fadeIn">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center">
                ...
                <p className="text-slate-400 font-black uppercase text-xs mb-2 tracking-[0.2em]">Foydalanuvchilar</p>
                <h3 className="text-6xl font-black text-slate-900">{users.length}</h3>
             </div>
             <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center">
                ...
                <h3 className="text-6xl font-black text-slate-900">{submissions.length}</h3>
             </div>
             <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center">
                ...
                <h3 className="text-6xl font-black text-slate-900">{teachers.length}</h3>
             </div>
          </div>
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 h-[500px]">
             <h3 className="text-2xl font-black mb-10 text-slate-900">{t('overall_stats')}</h3>
             <ResponsiveContainer width="100%" height="100%">
                <BarChart data={statsData} margin={{bottom: 40}}>
                   <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                   <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#0f172a', fontWeight: 'bold'}} dy={15} />
                   <YAxis axisLine={false} tickLine={false} tick={{fill: '#0f172a'}} />
                   <Tooltip />
      ...
      {activeTab === 'students' && (
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden animate-fadeIn">
           <h3 className="text-2xl font-black mb-8 text-slate-900">{t('user_report')}</h3>
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                 <thead>
                    <tr className="border-b-2 border-slate-50">
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('full_name')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('class_name')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('total_submitted')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">Holat</th>
                    </tr>
                 </thead>
                    ...
                    {users.filter(u => u.role === UserRole.STUDENT).map((u, i) => {
                       ...
                       return (
                          <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-900">
                             <td className="py-5 px-4 font-black text-slate-900">{u.fullName}</td>
                             <td className="py-5 px-4 text-blue-600 font-black text-sm">{u.className}</td>
                             <td className="py-5 px-4 font-bold text-slate-900">{userSubs.length}</td>
                             <td className="py-5 px-4">
                                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${userSubs.length > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      ...
      {activeTab === 'teachers' && (
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden animate-fadeIn">
           <h3 className="text-2xl font-black mb-8 text-slate-900">{t('rating')}</h3>
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                 <thead>
                    <tr className="border-b-2 border-slate-50">
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('teacher')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('subject')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('tasks')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">Reyting</th>
                    </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-50">
                    {teachers.map((t_teach, i) => (
                       <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-900">
                          <td className="py-5 px-4 font-black text-slate-900">{t_teach.fullName}</td>
                          <td className="py-5 px-4 font-bold text-slate-600 uppercase text-[10px] tracking-widest">{t_teach.subject}</td>
      ...
      {activeTab === 'users' && (
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 animate-fadeIn">
           <h3 className="text-2xl font-black mb-10 text-center text-slate-900">{t('admin_panel')}</h3>
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                 <thead>
                    <tr className="border-b-2 border-slate-50">
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('full_name')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('username')}</th>
                       <th className="py-6 px-4 font-black text-slate-500 uppercase text-[10px] tracking-widest">{t('role')}</th>
                    </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-50">
                    {users.map((u, i) => (
                       <tr key={i} className="hover:bg-slate-50 transition-colors text-slate-900">
                          ...
                          <td className="py-5 px-4">
                             <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${u.role === UserRole.ADMIN ? 'bg-purple-600 text-white' : u.role === UserRole.TEACHER ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}`}>
                                {u.role}
                             </span>
                          </td>
                       </tr>
